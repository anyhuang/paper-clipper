...
const getSegmentAngle = (segment: paper.Segment) => {
  ...
  const inPointAngleLocation = path.getLocationAt(
    segment.isFirst() ? path.length - 1 : segment.previous.location.offset,
  );
...
const paperClipperSimplify =
  (tolerance: number = paperClipperSimplifyTolerance) =>
  (targetPath: paper.Path): paper.Path => {
    ...
    const angledSegmentOffsets = angledSegments.map((segment) => segment.location.offset);
